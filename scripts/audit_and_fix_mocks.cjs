...
console.log('\n=== 2. CHECKING QUESTIONS FOR BRACKETS / ANSWER LEAKS ===');
const corrupted = db.prepare(`
  ...
  WHERE question_text LIKE '%(A)%' OR question_text LIKE '%(B)%' OR question_text LIKE '%(C)%' OR question_text LIKE '%(D)%'
     OR question_text LIKE '%(a)%' OR question_text LIKE '%(b)%' OR question_text LIKE '%(c)%' OR question_text LIKE '%(d)%'
...
`).all();
console.log(`Questions with answer leaks or brackets: ${corrupted.length}`);
if (corrupted.length > 0) {
  console.log('Sample corrupted:', corrupted.slice(0, 5));
}
...
console.log('Exam patterns:', patterns.map(p => ({ id: p.id, exam_id: p.exam_id, pattern_name: p.pattern_name, total_marks: p.total_marks })));

const sections = db.prepare('SELECT * FROM pattern_sections').all();
console.log('Pattern sections count:', sections.length);
sections.forEach(s => console.log(`   - Pattern [${s.pattern_id}] Section: ${s.section_name} -> ${s.question_count} Qs (subject_id: ${s.subject_id})`));
