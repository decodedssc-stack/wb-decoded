...
console.log('⚡ 1. Fast Deduplication of Questions Table...');

db.exec(`
  CREATE TEMP TABLE unique_q_ids AS
  SELECT MIN(id) as id FROM questions GROUP BY question_text;

  DELETE FROM questions WHERE id NOT IN (SELECT id FROM unique_q_ids);
  DROP TABLE unique_q_ids;
`);

const remainingCount = db.prepare('SELECT COUNT(*) as c FROM questions').get().c;
console.log('✅ Unique clean questions in database:', remainingCount);
...
englishTrans();
console.log('✅ Seeded 25 distinct English questions for WBCS 2023.');

// For the remaining 175 questions, fetch 100% DISTINCT questions from the database with ZERO overlap!
...
const remainingTrans = db.transaction(() => {
  for (const sec of SECTIONS_CONFIG) {
    ...
    // Fallback if needed from any unused distinct question
    if (added < sec.count) {
      const fallbackList = db.prepare(`SELECT id, question_text FROM questions ORDER BY id ASC`).all();
...
console.log('🎉 Successfully assembled WBCS 2023 with 200 COMPLETELY DISTINCT questions!');
